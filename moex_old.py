...
FILE_NAME = 'moex_token.txt'
MOEX_TOKEN_PATH = ''
home = os.path.expanduser('~')
for root, subdirs, files in os.walk(home, topdown=True):
    subdirs[:] = [sbd for sbd in subdirs if sbd[0] != '.' and sbd[0] != '_']
    if FILE_NAME in files:
        MOEX_TOKEN_PATH = os.path.join(root, FILE_NAME)

if MOEX_TOKEN_PATH == '':
    raise FileNotFoundError('File name: {} was not found'.format(FILE_NAME))
